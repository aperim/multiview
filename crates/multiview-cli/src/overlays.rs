...
impl OverlayBaker {
    ...
    /// renders its own face (ADR-W021: no first-wins).
    #[must_use]
    ...
    /// Replace the whole analog clock face **set** at runtime (ADR-W021 live
    ...
    pub fn draw_list(
        ...
        // (ADR-W021 — no first-wins); the model owns the time→angle math (the
