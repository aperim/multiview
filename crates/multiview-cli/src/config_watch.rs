...
/// dead watcher.
struct ActiveGuard(Arc<multiview_control::ConfigWatchStatus>);
    ...
    loop {
        ...
        if applied.as_ref() == Some(&now) {
            ...
            if incomplete_active {
                ...
                resync_all_stores(&state, &baseline);
...
/// What one file apply did: the human summary, the restart-only sections, and
/// how many engine commands were shed on a full bus (review M1: any shed
...
fn resync_all_stores(state: &AppState, config: &MultiviewConfig) {
    resync_store(state, &state.sources, &desired_sources(config));
    resync_store(state, &state.outputs, &desired_outputs(config));
    resync_store(state, &state.overlays, &desired_overlays(config));
    resync_store(state, &state.probes, &desired_probes(config));
    resync_store(state, &state.devices, &desired_devices(config));
    for device in &config.devices {
        state.device_status.ensure(&device.id);
    }
    resync_store(state, &state.sync_groups, &desired_sync_groups(config));
    if config.audio.is_some() {
        resync_audio(state, config);
    }
    let id = state
        .working_layout_id
        .clone()
