...
//!
//! The live-load probe in `mosaic-hal` samples each GPU at ~1-4 Hz; the engine
...
//!
//! **Decoupling.** This module deliberately does *not* depend on `mosaic-hal`'s
//! `DeviceLoad` type (the crate graph keeps telemetry a leaf). The poller reads
//! `mosaic_hal::DeviceLoad` and calls these handles; here we define only the
...
pub mod names {
    ...
    pub const GPU_COMPUTE_UTIL: &str = "mosaic_gpu_compute_utilization_ratio";
    /// GPU encoder-ASIC busy ratio (`0.0..=1.0`); registered only where exposed.
    pub const GPU_ENCODER_UTIL: &str = "mosaic_gpu_encoder_utilization_ratio";
    /// GPU decoder-ASIC busy ratio (`0.0..=1.0`); registered only where exposed.
    pub const GPU_DECODER_UTIL: &str = "mosaic_gpu_decoder_utilization_ratio";
    /// VRAM bytes in use (the authoritative VRAM-pressure numerator).
    pub const GPU_MEMORY_USED_BYTES: &str = "mosaic_gpu_memory_used_bytes";
    /// Total VRAM bytes (the VRAM-pressure denominator).
    pub const GPU_MEMORY_TOTAL_BYTES: &str = "mosaic_gpu_memory_total_bytes";
    /// Active NVENC encode-session count (Mosaic's own tracked figure).
    pub const GPU_ENCODER_SESSIONS_ACTIVE: &str = "mosaic_gpu_encoder_sessions_active";
    /// Discovered per-system NVENC concurrent-session ceiling.
    pub const GPU_ENCODER_SESSION_CEILING: &str = "mosaic_gpu_encoder_session_ceiling";
    /// Whole-system CPU busy ratio (`0.0..=1.0`), all cores aggregated.
    pub const CPU_UTIL: &str = "mosaic_cpu_utilization_ratio";
}
