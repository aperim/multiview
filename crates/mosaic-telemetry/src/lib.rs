//! # mosaic-telemetry
//!
//! Observability primitives for the Mosaic engine: a `tracing` subscriber
...
//! not run an HTTP server (that is `mosaic-control`'s job) and it never installs
...
//! * `snmp` *(feature `snmp`)* — the SNMP trap path: the Mosaic enterprise
