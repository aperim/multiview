...
//!
//! * The object-identifier model ([`Oid`]), the SNMP value subset Mosaic emits
//!   ([`VarBindValue`]), a single `OID = value` binding ([`VarBind`]), and the
//!   notification payload ([`Trap`]) / wire PDU ([`TrapPdu`]).
//! * The Mosaic enterprise MIB OIDs in [`mib`] (the notification and object
...
/// The subset of SNMP/`SMIv2` value types Mosaic binds in a trap.
///
...
/// The Mosaic enterprise SNMP MIB: the object/notification OIDs every emitted
/// trap is keyed by.
///
...
pub mod mib {
    ...
    /// Placeholder IANA Private Enterprise Number for Mosaic.
    ///
    ...
    /// Sub-arc of the enterprise root holding alarm objects (`mosaicAlarm`).
    const ALARM_GROUP: u32 = 1;
    /// Sub-arc holding notification (trap) types (`mosaicNotifications`).
    const NOTIF_GROUP: u32 = 0;

    /// Internal helper: build a Mosaic OID, falling back to the bare enterprise
    ...
    /// The Mosaic enterprise root OID (`1.3.6.1.4.1.<PEN>`).
    #[must_use]
    pub fn enterprise_oid() -> Oid {
        mosaic_oid(&[])
    }

    /// `mosaicAlarmRaise` notification OID.
    #[must_use]
    pub fn alarm_raise_oid() -> Oid {
        mosaic_oid(&[NOTIF_GROUP, 1])
    }

    /// `mosaicAlarmClear` notification OID.
    #[must_use]
    pub fn alarm_clear_oid() -> Oid {
        mosaic_oid(&[NOTIF_GROUP, 2])
    }

    /// `mosaicAlarmSeverity` object OID (the X.733 severity index).
    #[must_use]
    pub fn severity_object_oid() -> Oid {
        mosaic_oid(&[ALARM_GROUP, 1])
    }

    /// `mosaicAlarmKind` object OID (the probe/fault class label).
    #[must_use]
    pub fn kind_object_oid() -> Oid {
        mosaic_oid(&[ALARM_GROUP, 2])
    }

    /// `mosaicAlarmScope` object OID (a human-readable scope label).
    #[must_use]
    pub fn scope_object_oid() -> Oid {
        mosaic_oid(&[ALARM_GROUP, 3])
    }

    /// `mosaicAlarmId` object OID (the stable alarm instance id).
    #[must_use]
    pub fn alarm_id_object_oid() -> Oid {
        mosaic_oid(&[ALARM_GROUP, 4])
    }

    /// `mosaicAlarmDwellMillis` object OID (how long the condition has dwelt).
    #[must_use]
    pub fn dwell_object_oid() -> Oid {
        mosaic_oid(&[ALARM_GROUP, 5])
    }
...
/// A stable, lower-snake label for an [`AlarmKind`](mosaic_core::alarm::AlarmKind),
/// bound into the `mosaicAlarmKind` var-bind. Matches the serde tag so the SNMP
/// and JSON/event surfaces agree.
fn alarm_kind_label(kind: mosaic_core::alarm::AlarmKind) -> &'static str {
...
/// A human-readable scope label bound into the `mosaicAlarmScope` var-bind.
fn scope_label(scope: &mosaic_core::alarm::AlarmScope) -> String {
    use mosaic_core::alarm::AlarmScope;
    match scope {
