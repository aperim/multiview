...
use mosaic_telemetry::metrics::{Labels, MetricKind, MetricsRegistry};

#[test]
fn counter_registers_and_increments() {
    let reg = MetricsRegistry::new();
    let frames_out = reg.counter("mosaic_frames_out_total", Labels::empty());
...
fn same_name_and_labels_return_the_same_counter() {
    ...
    let a = reg.counter("mosaic_dropped_total", labels.clone());
    let b = reg.counter("mosaic_dropped_total", labels);
    a.increment(3);
...
fn different_labels_are_distinct_series() {
    let reg = MetricsRegistry::new();
    let t0 = reg.counter("mosaic_dropped_total", Labels::new().with("tile", "0"));
    let t1 = reg.counter("mosaic_dropped_total", Labels::new().with("tile", "1"));
    t0.increment(2);
    t1.increment(7);
    assert_eq!(t0.get(), 2);
...
fn gauge_sets_and_reads_back() {
    let reg = MetricsRegistry::new();
    let fps = reg.gauge("mosaic_tile_fps", Labels::new().with("tile", "2"));
    fps.set(29.97);
...
fn histogram_rejects_observation_count_overflow_gracefully() {
    ...
    let reg = MetricsRegistry::new();
    let h = reg.histogram("mosaic_decode_seconds", Labels::empty(), &[0.01, 0.1, 1.0]);
    for _ in 0..1000 {
        h.observe(0.05);
    }
    let snap = h.snapshot();
...
fn registry_describes_registered_series() {
    let reg = MetricsRegistry::new();
    let _ = reg.counter("mosaic_frames_in_total", Labels::empty());
    let _ = reg.gauge("mosaic_vram_bytes", Labels::new().with("backend", "cuda"));
    let _ = reg.histogram("mosaic_composite_seconds", Labels::empty(), &[0.01]);
