...
use mosaic_cli::run::HeadlessEngine;
use mosaic_config::MosaicConfig;
use mosaic_engine::{CooperativePacer, ManualTimeSource};
...
fn load(name: &str) -> MosaicConfig {
    let text = std::fs::read_to_string(example(name)).expect("read example");
    let cfg = MosaicConfig::load_from_toml(&text).expect("parse example");
...
codec = "h264"
"##;
    let cfg = MosaicConfig::load_from_toml(toml).expect("parse small config");
...
async fn headless_frame_pts_advances_by_exactly_one_tick_period() {
    ...
    let first = mosaic_core::time::MediaTime::from_tick(0, cadence);
    let last = mosaic_core::time::MediaTime::from_tick(29, cadence);
...
async fn realtime_headless_run_paces_to_wall_clock() {
    ...
    let period_ns = mosaic_core::time::MediaTime::from_tick(1, cadence).as_nanos();
    let expected =
