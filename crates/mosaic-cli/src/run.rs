//! The `mosaic run` subcommand and its **headless software engine**.
//!
...
impl HeadlessEngine {
    ...
    pub fn build(config: &MosaicConfig) -> Result<Self, RunError> {
    ...
    /// [`mosaic_engine::CooperativePacer`]: before driving, the manual clock is jumped past the
    ...
    async fn drive<P: Pacer>(
        ...
        let state_of = |frame: &mosaic_engine::CompositedFrame| HeadlessState {
...
/// # Errors
///
/// Returns the compositor [`mosaic_compositor::Error`] if the geometry is
...
) -> Result<Nv12Image, mosaic_compositor::Error> {
