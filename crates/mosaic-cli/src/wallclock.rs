...
//! platform Mosaic targets (Linux + macOS) that *is* `CLOCK_REALTIME` — the
...
//!
//! The overlay model ([`mosaic_overlay::clock::TimeRef`]) carries a
...
//! needs `unsafe` FFI — which `mosaic-cli` forbids ([`forbid(unsafe_code)`]) — so
...
use mosaic_overlay::clock::{RefSource, RefStatus, TimeRef, WallTime};
