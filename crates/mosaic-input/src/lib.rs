...
//! **Mosaic** live video mosaic engine.
//!
...
//!   onto [`mosaic_core::tally`]. The sockets that carry them are a later
...
//!   `TestPatternSource`) over `mosaic-ffmpeg`'s **safe** wrappers, implementing
//!   [`source::FrameProducer`]. All FFI is owned by `mosaic-ffmpeg`; this crate
//!   stays `unsafe_code = forbid`.
//!
//! All timing math is exact (i64 nanoseconds / i128 intermediates / exact
//! rationals via [`mosaic_core::time`]) — **never** float fps.
//!
