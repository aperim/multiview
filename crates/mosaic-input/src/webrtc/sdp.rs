...
//! Mosaic negotiates — the session version line, each `m=` media description, its
...
pub enum MediaKind {
    ...
    Application,
    /// Any other media kind, preserving nothing (Mosaic ignores these).
    Other,
}
...
/// A parsed SDP session description (the subset Mosaic negotiates).
    ...
    pub fn negotiate_answer(
        ...
        for m in &self.media {
            let supported = match m.kind {
                ...
                // Mosaic does not negotiate data/other sections; skip them.
