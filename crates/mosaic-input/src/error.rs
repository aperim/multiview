...
//!
//! `mosaic-input` owns ingest, timestamp normalization, jitter buffering,
...
//! [`mosaic_core::Error::Input`] arm at the crate boundary.
use thiserror::Error;
...
pub enum Error {
    ...
    /// detail string carries the underlying `mosaic-ffmpeg` error so the pure-Rust
...
#[cfg(feature = "ffmpeg")]
impl From<mosaic_ffmpeg::FfmpegError> for Error {
    fn from(value: mosaic_ffmpeg::FfmpegError) -> Self {
...
impl From<Error> for mosaic_core::Error {
    fn from(value: Error) -> Self {
        mosaic_core::Error::Input(value.to_string())
    }
}
