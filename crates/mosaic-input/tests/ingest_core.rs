...
use mosaic_framestore::{TileRead, TileStore};
use mosaic_input::normalize::WrapBits;
use mosaic_input::source::{FrameProducer, IngestConfig, IngestPump, ProducedFrame, StoredFrame};
...
impl FrameProducer for ScriptedProducer {
    fn next_frame(&mut self) -> mosaic_input::Result<Option<ProducedFrame>> {
        match self.frames.pop_front() {
            Some(f) => Ok(Some(f)),
            None if self.fault_at_end => {
                Err(mosaic_input::Error::Ingest("scripted fault".to_owned()))
            }
            None => Ok(None),
        }
    }
...
fn producer_fault_propagates_as_error_not_panic() {
    ...
    match pump.pump_one(&mut producer, &store, MediaTime::ZERO) {
        Err(mosaic_input::Error::Ingest(msg)) => assert!(msg.contains("scripted fault")),
