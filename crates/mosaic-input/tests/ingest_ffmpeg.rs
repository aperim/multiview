...
//! LGPL-clean), ingest it through the real `mosaic-ffmpeg` safe demux/decode
...
use mosaic_input::libav::{FileSource, TestPatternSource, TestPatternSpec};
use mosaic_input::source::{FrameProducer, IngestConfig, IngestPump, StoredFrame};
...
/// path. Mirrors the mosaic-ffmpeg test fixtures so this suite is independent.
...
fn open_missing_file_is_typed_error_not_panic() {
    ...
    let missing = Path::new("/nonexistent/mosaic-input/does-not-exist.mkv");
    match FileSource::open(missing) {
        Ok(_) => panic!("opening a missing file must fail"),
        Err(mosaic_input::Error::Ingest(msg)) => {
            assert!(
