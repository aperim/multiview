...
use mosaic_input::jitter::ReorderBuffer;
use mosaic_input::normalize::{PtsNormalizer, WrapBits};
use mosaic_input::pacer::Pacer;
...
proptest! {
    ...
    fn prop_wrap_unwrap_monotonic_across_boundary(
        ...
        let step_ns = mosaic_core::time::rescale(
            step,
            ts_tb(),
        ...
        let total_true_ns = mosaic_core::time::rescale(
    ...
    fn prop_pacer_release_in_pts_order(
        ...
        let mut p = Pacer::new(mosaic_input::pacer::PacerConfig::default());
        // Anchor.
