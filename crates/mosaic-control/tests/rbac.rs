...
use mosaic_control::{authorize_output, Action, ControlError, Principal, Role};
...
fn output_and_object_scopes_are_independent() {
    ...
    // Object guard.
    assert!(mosaic_control::authorize_object(&p, "layout-1").is_ok());
    assert!(mosaic_control::authorize_object(&p, "layout-2").is_err());
