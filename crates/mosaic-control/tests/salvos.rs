...
async fn arm_then_take_returns_202_and_reaches_the_engine() {
    ...
    match &drained[0] {
        mosaic_control::Command::ArmSalvo { op, salvo, head } => {
    ...
    match &drained[1] {
        mosaic_control::Command::TakeSalvo { op, salvo, .. } => {
...
async fn take_with_head_query_scopes_the_command() {
    ...
    let drained = h.commands.try_drain();
    match &drained[0] {
        mosaic_control::Command::TakeSalvo { head, .. } => {
...
fn openapi_salvo_mirror_matches_the_config_salvo_serde_shape() {
    ...
    use mosaic_control::openapi_schemas::SalvoDoc;
...
async fn output_scoped_role_permits_head_inside_its_allowlist() {
    ...
    for cmd in &drained {
        let head = match cmd {
            mosaic_control::Command::ArmSalvo { head, .. }
            | mosaic_control::Command::TakeSalvo { head, .. }
            | mosaic_control::Command::CancelSalvo { head, .. } => head.as_deref(),
...
async fn cancel_returns_202_and_carries_the_salvo() {
    ...
    let drained = h.commands.try_drain();
    match &drained[0] {
        mosaic_control::Command::CancelSalvo { salvo, .. } => {
            assert_eq!(salvo.as_deref(), Some("wide"));
        }
