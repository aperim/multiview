...
use mosaic_control::{Device, MediaFormat, Node, Receiver, ResourceCore, Sender, TransportParams};
use support::{
...
async fn node_self_is_served_to_an_authenticated_reader() {
    ...
    assert_eq!(body["hostname"], "mosaic.local");
}
...
fn openapi_document_advertises_the_nmos_routes() {
    use mosaic_control::openapi::ApiDoc;
    use utoipa::OpenApi;
