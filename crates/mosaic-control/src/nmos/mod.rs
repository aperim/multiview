...
//!
//! Mosaic speaks the NMOS suite so it appears as a first-class node in an IP
...
//!
//! This module adds the [`NmosRegistry`] (the control-plane store of Mosaic's own
...
/// The control-plane registry of Mosaic's own NMOS resources + connection state.
///
/// Holds the node Mosaic advertises, its devices, the senders (program/preview
...
impl NmosRegistry {
    ...
    /// Set the node resource Mosaic advertises.
...
#[cfg_attr(
    ...
    utoipa::path(
        ...
        tag = "nmos",
        responses(
            (status = 200, description = "The Mosaic NMOS node resource.", body = crate::nmos::is04::Node),
