...
//!
//! The pure model maps both ways to Mosaic's own vocabulary: a `boolean` IS-07
...
//! a lamp from Mosaic's arbitrated state.
//!
//! ## Transports
//!
...
//! always compiled and always tested. With the feature off, Mosaic still speaks
...
///
/// This is Mosaic's own vocabulary for a Boolean tally/contact-closure event,
...
/// the target, so an external IS-07 receiver can light a lamp from Mosaic's
...
/// Recover a [`TallyState`](mosaic_core::tally::TallyState)'s lamp colour from an
...
pub mod mqtt {
    ...
    //! the pure message model is always available, so Mosaic always speaks IS-07
