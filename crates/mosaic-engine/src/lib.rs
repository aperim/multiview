...
//! `mosaic_engine`.
//!
//! This crate is where Mosaic's two load-bearing invariants live, and it treats
//! them as hard rules:
//!
...
//!   any input: `out_pts = f(tick)` via [`mosaic_core::time::MediaTime::from_tick`],
...
//!   sense→estimate→plan→apply over the `mosaic-hal` cost model and hysteresis
...
//!   [`MediaTime`](mosaic_core::time::MediaTime)** — best-effort samplers that
...
//!   over an injected [`MediaTime`](mosaic_core::time::MediaTime)** that *return*
...
//!   an injected [`MediaTime`](mosaic_core::time::MediaTime) (on a miss-threshold
...
//! CPU reference compositor from `mosaic-compositor`, and the clock is driven by
...
pub use alarm::{
    AlarmHysteresis, AlarmStateMachine, AlarmTransition, BoolOp, PenaltyAction, PenaltyBox,
    PenaltyConfig, PenaltyState, Phase, RollupNode, VirtualAlarm,
};
