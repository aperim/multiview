...
//! shared alarm vocabulary in [`mosaic_core::alarm`] (ADR-MV001).
//!
...
//! [`MediaTime`](mosaic_core::time::MediaTime)** — exactly the pattern used by
//! the tile failure-ladder ([`mosaic_framestore::state`]) and the alert card
//! ([`mosaic_overlay::alert`]). There are no timers, no sleeps, no channels and
