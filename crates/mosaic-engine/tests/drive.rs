...
use mosaic_engine::clock::Tick;
use mosaic_engine::{CompositorDrive, OutputClock};
use mosaic_framestore::{TileStore, TileThresholds};
...
fn holds_last_good_frame_when_a_producer_dies() {
    ...
    let store_a = Arc::new(TileStore::<Nv12Image>::new(
        "cam-a",
        thresholds,
        mosaic_framestore::NoSignalPolicy::Slate,
    ));
