...
//!
//! `mosaic-config` owns parsing (TOML/JSON), grid solving, and semantic
...
//! the workspace-wide [`mosaic_core::Error`] are provided at the boundary.
...
impl From<ConfigError> for mosaic_core::Error {
    fn from(err: ConfigError) -> Self {
        mosaic_core::Error::Config(err.to_string())
    }
}
