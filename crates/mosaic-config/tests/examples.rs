...
fn examples_dir() -> PathBuf {
    // CARGO_MANIFEST_DIR = crates/mosaic-config
...
fn every_example_loads_and_validates() {
    ...
    for name in EXAMPLES {
        ...
        let cfg =
            MosaicConfig::load_from_toml(&text).unwrap_or_else(|e| panic!("load {name}: {e}"));
        cfg.validate()
...
fn every_example_grid_solves_to_a_valid_core_layout() {
    ...
    for name in EXAMPLES {
        ...
        let cfg = MosaicConfig::load_from_toml(&text).unwrap();
        let layout = cfg
            .solve_layout()
...
fn examples_round_trip_through_json_and_toml() {
    ...
    for name in EXAMPLES {
        ...
        let from_json =
            MosaicConfig::load_from_json(&json).unwrap_or_else(|e| panic!("{name} from_json: {e}"));
        ...
        let from_toml = MosaicConfig::load_from_toml(&toml_text)
...
fn known_fps_values_parse_to_exact_rationals() {
    ...
    let cfg = MosaicConfig::load_from_toml(&std::fs::read_to_string(dir.join("2x2.toml")).unwrap())
        .unwrap();
    assert_eq!(cfg.canvas.fps.rational(), Rational::FPS_29_97);

    let cfg = MosaicConfig::load_from_toml(&std::fs::read_to_string(dir.join("3x3.toml")).unwrap())
        .unwrap();
    assert_eq!(cfg.canvas.fps.rational(), Rational::FPS_25);

    let cfg =
        MosaicConfig::load_from_toml(&std::fs::read_to_string(dir.join("1plus5.toml")).unwrap())
            .unwrap();
