...
fn base_document_is_valid() {
    let cfg = MosaicConfig::load_from_toml(BASE).unwrap();
...
fn dangling_cell_source_input_id_is_rejected() {
    ...
    let cfg = MosaicConfig::load_from_toml(&bad).unwrap();
...
fn float_fps_is_rejected_at_parse_time() {
    ...
    assert!(
        MosaicConfig::load_from_toml(&bad).is_err(),
...
fn zero_denominator_fps_is_rejected() {
    ...
    if let Ok(cfg) = MosaicConfig::load_from_toml(&bad) {
        assert!(
            cfg.validate().is_err(),
...
codec = "h264"
"##;
    let cfg = MosaicConfig::load_from_toml(doc).unwrap();
