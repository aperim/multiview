...
fn captions_absent_is_none_and_document_still_validates() {
    ...
    let cfg = MosaicConfig::load_from_toml(&doc_with_captions("")).expect("parse");
...
fn teletext_page_selector_round_trips_through_toml_and_json() {
    let cfg = MosaicConfig::load_from_toml(&doc_with_captions(
    ...
    assert_eq!(MosaicConfig::load_from_json(&json).expect("from_json"), cfg);

    let toml_text = cfg.to_toml().expect("to_toml");
    assert_eq!(
        MosaicConfig::load_from_toml(&toml_text).expect("from_toml"),
        cfg
    );
}
...
fn every_selector_mode_parses_to_its_variant() {
    ...
    for (line, want) in cases {
        let cfg = MosaicConfig::load_from_toml(&doc_with_captions(line)).expect("parse");
...
fn a_mode_less_captions_table_is_rejected() {
    ...
    let err = MosaicConfig::load_from_toml(&doc_with_captions("captions = { page = 801 }"));
