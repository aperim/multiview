...
//! is the only part of `mosaic-audio` that depends on libav — and it does so
//! **without touching it directly**: all demux/decode/resample lives in
//! [`mosaic_ffmpeg`], which exposes a pure-Rust seam
//! ([`mosaic_ffmpeg::AudioFileDecoder`] yielding
//! [`mosaic_ffmpeg::AudioSamplesF32`]) with no libav type in its signature.
//! `mosaic-audio` stays `unsafe_code = forbid` and never names a binding type
//! (CLAUDE.md §7).
//!
...
use mosaic_ffmpeg::{AudioFileDecoder as FfDecoder, AudioSamplesF32};
...
///
/// Wraps the pure [`mosaic_ffmpeg::AudioFileDecoder`] seam: every block is
