...
//! `decode(encode(m)) == m`. The encoder lives in `mosaic-output`; the decoder
//! in `mosaic-input` (pulled as a dev-dependency only). The two crates define
...
use mosaic_input::tsl as dec;
use mosaic_output::tsl as enc;
