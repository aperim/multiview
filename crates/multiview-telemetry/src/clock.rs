//! Clock-layer servo telemetry (DEV-C4, ADR-M010): the disciplined-reference
//! servo's **offset (ns)** and **frequency correction (ppb)**, plus the
//! display-audio buffer servo's **resample-ratio correction (ppm)**, exported
//! through the dependency-free [`MetricsRegistry`] as lock-free gauges.
//!
