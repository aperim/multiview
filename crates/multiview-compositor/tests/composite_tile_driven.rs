//! Equivalence tests for the **tile-driven** CPU composite kernel (the
//! O(pixels × covering-tiles) rewrite of the old O(pixels × all-tiles) kernel;
//! the discovery critic's #1 perf finding).
//!
...
//!
//! The per-tick composite *budget* + coverage-scaling perf gates live in a
//! dedicated test binary (`composite_perf_budget.rs`) so libtest never schedules
//! them alongside these correctness composites (a shared binary oversubscribed
//! the CPU and inflated the perf median; #75).
#![allow(
    clippy::unwrap_used,
    clippy::expect_used,
    clippy::panic,
