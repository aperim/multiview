...
//!
//! No network, no muxing: the SubRip / WebVTT / mov_text decoders consume the
//! cue **body** as packet payload and take timing from the packet PTS, so a
...
fn one_text(cues: Vec<CaptionCue>) -> (Vec<String>, i64, i64) {
...
fn subrip_packet_decodes_to_text_cue_with_rebased_start() {
    ...
    assert_eq!(end_ns, 1_000_000_000 + DEFAULT_HOLD_NS, "bounded hold applied");
}
...
fn empty_packet_yields_no_cue_and_never_panics() {
    ...
    let mut dec = CaptionDecoder::for_embedded(CaptionSource::SubRip, ms_time_base())
        .expect("open subrip decoder");
    let cues = dec.decode_bytes(b"", Some(1_000)).expect("empty is not an error");
    assert!(cues.is_empty(), "empty packet -> no cue");
}

#[test]
fn malformed_packet_yields_no_cue_and_never_panics() {
    // A structurally-invalid caption packet must degrade gracefully: it either
    // yields no cue or a typed decode error — never a panic and never a stall.
    let mut dec = CaptionDecoder::for_embedded(CaptionSource::MovText, ms_time_base())
        .expect("open mov_text decoder");
    // A length prefix (0xFFFF) far larger than the 1-byte body: the decoder
    // rejects or ignores it rather than reading out of bounds.
    let result = dec.decode_bytes(&[0xFF, 0xFF, 0x41], Some(1_000));
    match result {
        Ok(cues) => assert!(cues.is_empty(), "malformed -> no cue"),
        Err(_) => { /* a typed decode error is also graceful (no panic/stall) */ }
    }
}
