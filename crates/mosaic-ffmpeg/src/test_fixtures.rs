...
//!
//! This lives in `mosaic-ffmpeg` (the **only** crate allowed `unsafe` for raw
//! libav* FFI) so the strictly-`forbid(unsafe)` `mosaic-cli` test can call it
//! across the crate boundary; the `mosaic-ffmpeg` demux test uses it too. It is
