...
fn present_device_yields_a_valid_capability_with_max_resolution_and_formats() {
    let caps = DeviceCaps {
        max_resolution: mosaic_hal::Resolution::UHD4K,
    ...
    assert_eq!(decode.max_resolution, mosaic_hal::Resolution::UHD4K);
...
fn present_device_without_a_stage_reports_unavailable_for_that_stage() {
    let caps = DeviceCaps {
        max_resolution: mosaic_hal::Resolution::HD1080,
...
fn probe_resolves_cleanly(kind: BackendKind) {
    for stage in Stage::ALL {
        match mosaic_hal::probe::probe(kind, stage) {
            Ok(cap) => {
