...
fn live_apply_upsert(
    ...
    if previous.is_some_and(|prev| state.live_sources.is_live(&prev.kind)) {
        // Live-appliable -> non-live kind change: stop the running producer
        // now; the stored document applies on restart. A shed submit is
        // surfaced (never silent): the stale producer keeps running until
        ...
        if let Err(err) = state.commands.try_submit(Command::RemoveSource {
            ...
            tracing::warn!(
                source = %source.id,
                error = %err,
                "kind-change RemoveSource shed: the running producer keeps \
                 rendering the old picture until restart"
            );
        }
    }
    ApplyMode::Restart
