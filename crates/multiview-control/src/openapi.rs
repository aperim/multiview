...
/// The data behind [`ApiDoc::rest_routes`]: every REST route, in
/// registration order (kept as a module-level const so the accessor stays a
/// one-liner as the surface grows).
const REST_ROUTES: &[(&str, &str)] = &[
    ...
    // Ephemeral Cast sessions (DEV-D2, ADR-M011).
