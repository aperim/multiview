...
- **Status:** CODE SHIPPED (branch `ship/dev-b5-node-mode`), hardware legs pending. `multiview node` is a `Command` variant driving the **standard full pipeline** over a lowered node document (`multiview-config/src/node.rs`: one RTSP/SRT/HLS/TS ingest → one full-canvas cell carrying `on_loss` → one `Output::Display` per head; no control plane — enrollment is DEV-B6), so ingest pacer/jitter/normalize/reconnect + the framestore ladder + the B1–B4 display sink/audio path are reused **unchanged**; a build without `display-kms`+`ffmpeg` rejects the subcommand with a clear error (B1 precedent). Deployment: `deploy/multiview-node.service` (`Type=notify`, `User=multiview` + `video render audio`, masked-getty + implicit-DRM-master + fbcon-restore documented in `deploy/display-node.md`) over a dependency-free `sdnotify.rs` (one non-blocking `AF_UNIX` datagram; READY=1 at the **first frame boundary**, STOPPING=1 on the way out, and a **tick-gated** watchdog — pings at half `WatchdogSec` only while the output tick counter advances, so a stalled clock gets restarted: invariant #1 enforced by the supervisor). Hotplug: `display/hotplug.rs` (always-compiled parse/debounce/monitor over a `UeventSource` seam; udevd `libudev`-magic rejected) + the real `NETLINK_KOBJECT_UEVENT` socket in `kms.rs` bound to the **kernel** group (rustix unified on 1.x — net dep reduction), with the rootless case detected via the `/proc/self/uid_map` heuristic (the socket binds fine in a user-ns but never receives) → `force_probe` polling at the node document's `hotplug.poll_secs`; the flip loop consumes the re-probe flag between flips and re-validates (`TEST_ONLY`) + re-lights on disconnect→reconnect (CI-tested over the scripted backend; `reprobes`/`relights` counters exported). The C2 boundary is explicit: `timing.link_offset_ms` is recorded + validated, presentation rides the sink's repeat/drop reconciliation until DEV-C2. All gates green (default + `display-kms` + `ffmpeg,display-kms` check/clippy/test, fmt, deny). Hardware legs remain: t630 24 h node soak (RTSP→VAAPI decode→scanout+audio), real connector unplug/replug on glass, rootful-container uevent delivery on the docker host.
