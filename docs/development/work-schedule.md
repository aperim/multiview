...
- [ ] **OUT-4b** `M` — NDI output: cli runnable egress + roundtrip test  ·  _deps: OUT-4_  · _wire `RunnableOutput::Ndi` into `pipeline.rs` build_outputs/run_one_output (canvas → `NdiOutput::send_canvas` off the hot path), add `[system.ndi]` accept-license config field, and the named `tests/ndi_output_roundtrip.rs` (feature `ndi`, in-process Finder/Receiver, content-aware ramp survival, #10 re-assert). Hardware-gated where a real runtime is needed._
...
- [ ] **ENG-1c** `S` — Make `live_blocked_sink_stays_bounded_and_never_stalls` (streaming_encode) deterministic across encoder speeds  ·  _deps: —_  · _the box (ffmpeg 5.1, slower mpeg2 encoder) emits too few packets in the window to overflow the wedged sink's bounded queue, so `dropped > 0` fails (passes on ffmpeg 7.1 here). The bounded-drop-oldest queue is correct; the TEST over-specifies. Fix: feed enough packets to DETERMINISTICALLY overflow regardless of encoder latency (e.g. inject packets directly / shrink the queue / decouple from real encode timing) — do NOT weaken the never-stalls assertion._
