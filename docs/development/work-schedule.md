...
> **Native Dante: NOT a planned feature** ([ADR-T010](../decisions/ADR-T010.md)). The earlier `DANTE-1..5` native-SDK tasks are removed — native Dante depends on an Audinate SDK that, as conveyed to us, is not offered to open-source/community projects without a paid commercial arrangement, and AES67 / ST 2110-30 (above) already reaches Dante facilities through Dante's AES67 mode. There is no `multiview-dante-sys` leaf and no native-Dante feature in the build.
