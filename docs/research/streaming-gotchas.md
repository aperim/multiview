...
**HIGH RISK — WebVTT subtitle rendition can kill the video ([ADR-T011](../decisions/ADR-T011.md)).** A master with an `EXT-X-MEDIA:TYPE=SUBTITLES` rendition (e.g. ABC News AU) is folded into the **one shared `AVFormatContext`** libav opens for the video. Whether the rendition is surfaced is strictness-gated (`hls.c` `new_rendition` drops it when `strict_std_compliance > experimental`); if it *is* surfaced, a corrupt/404/expired `.vtt` aborts the open or makes `av_read_frame` return that rendition's error for the **whole context** — blacking out the tile. **Fix:** the main demuxer discards every *unrouted* subtitle stream (`AVDISCARD_ALL`, keyed on `medium == Subtitle`, before the first read — `recheck_discard_flags` is one-shot), hardens the HLS open (`strict=normal`, `seg_max_retry`, a sane `protocol_whitelist`; never widen `allowed_extensions`), and runs HLS WebVTT through a **separate isolated reader** (its own context, `allowed_extensions=ALL`, supervised-reconnected). Audio renditions are never touched; a routed in-container DVB-sub stream is kept.
