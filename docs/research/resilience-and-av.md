...
- `mosaic-clock` — monotonic output/program clock, tick generation, drift discipline.
- `mosaic-framestore` — lock-free per-tile last-good-frame store + per-tile state machine.
- `mosaic-compositor` — wgpu render graph, scene-graph double-buffer/atomic swap, overlay layer stack, RGBA→NV12, readback ring, device-lost `rebuild()`.
- `mosaic-overlay` — cosmic-text/glyphon + Vello/SDF, libass burn-in, premultiplied atlas, color management. Backend-agnostic output (atlas + quad list).
- `mosaic-audio` — per-input resample/sync, anullsrc fill, discrete-track + program-bus routing, loudnorm; metering (`ebur128`) on isolated threads.
- `mosaic-input` — process-isolated FFI ingest workers (libav, interrupt callbacks, timeouts).
- `mosaic-encoder` — process-isolated encoder workers, hot-standby, pinned config, proactive recycle.
- `mosaic-mux` — TS/HLS/SRT/NDI/RTMP/RTSP muxers, continuity config, per-format capability matrix.
- `mosaic-supervisor` — supervision tree, backoff, circuit breakers, watchdog, IPC.
- `mosaic-ipc` — shared-memory rings, control/metadata transport.
- `mosaic-control` / `mosaic-api` — OpenAPI surface, config validation against capability matrix, WebSocket telemetry.
- `mosaic-chaos` / `mosaic-validate` — fault-injection harness, output-validity probe, SLO metrics.
