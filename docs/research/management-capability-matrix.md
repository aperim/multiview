...
- **`/v1/program`, `/v1/program/preview`** — Program/Preview bus (cue-then-take, cut/crossfade). **Superseded (design, 2026-06-11):** the single-bus cue-then-take sketch is replaced by the production-switcher M/E model — per-M/E PGM/PVW buses under `/api/v1/switcher/*` ([ADR-0054](../decisions/ADR-0054.md), [ADR-W021](../decisions/ADR-W021.md); rows in §2.11). Retained here unedited as the as-designed history, per repo convention (annotate, don't delete).
...
Every edit is classified and surfaced **before apply** via `POST /v1/outputs/{id}/plan` (dry-run) and `POST /v1/program:take?dry_run=true` (the `program:take` dry-run is superseded by the per-action `/plan` verbs of the switcher surface — [ADR-W021](../decisions/ADR-W021.md), §2.11):
...
| Source.lifecycle | prewarm/cue | POST .../prewarm · POST .../cue | Row > Cue/Pre-warm; Program bar | Hot | Off-air decode then cue to Preview. In the switcher model, cue/pre-warm **is** the PVW arm mechanism ([ADR-P007](../decisions/ADR-P007.md), §2.11). |
...
| Transition | cut\|crossfade | POST .../:apply {transition,duration_ms} | Take bar > Cut/Auto | Hot | Double-buffered swap. **Superseded (2026-06-11):** generalized to the full transition family (mix/dip/wipe/DVE/stinger) on the M/E model — [ADR-0055](../decisions/ADR-0055.md), §2.11. |
| Transition | crossfade duration/curve | POST .../:apply {duration_ms,curve} | Take bar > Transition settings | Hot | **Superseded:** `duration_ms` sketch replaced by integer `rate_frames`/`rate_ms` via exact rationals — [ADR-0055](../decisions/ADR-0055.md)/[ADR-T015](../decisions/ADR-T015.md), §2.11. |
| Transition | keyframe-aligned | POST .../:apply {align:'keyframe'} | Take bar > Advanced | Hot | Avoids artifacts. **Superseded** into the switcher take path (§2.11). |
| Program/Preview | preview (cue) | PUT /v1/program/preview {layout_id} | Send to Preview; PVW | Hot | Pre-warms. **Superseded:** PVW is per-M/E switcher state; cue/pre-warm is the PVW arm — [ADR-0054](../decisions/ADR-0054.md)/[ADR-P007](../decisions/ADR-P007.md), §2.11. |
| Program/Preview | take | POST /v1/program:take {transition} | TAKE button | Hot | Core live path. **Superseded:** cut/auto per M/E under `/api/v1/switcher/mix-effects/{id}` — [ADR-W021](../decisions/ADR-W021.md), §2.11. |
| Program/Preview | program binding | GET/PUT /v1/program | Program indicator; PGM | Hot | **Superseded:** M/E↔program binding is switcher config; live PGM state engine-owned, mirrored read-only — [ADR-M012](../decisions/ADR-M012.md), §2.11. |
...
| Layout I/O | diff/dry-run | POST .../:diff; POST /v1/program:take?dry_run | Take bar > Preview changes | RO | reset_required flag. `program:take?dry_run` superseded by the per-action `/plan` dry-runs (§2.11, [ADR-W021](../decisions/ADR-W021.md)). |
...
### 2.11 PRODUCTION SWITCHER & MEDIA PLAYOUT *(designed 2026-06-11 — not implemented)*

**Designed capability area — nothing in this section is implemented yet.** Designed in [production-switcher](production-switcher.md) + [media-playout](media-playout.md); decided in [ADR-0054](../decisions/ADR-0054.md)–[ADR-0059](../decisions/ADR-0059.md), [ADR-M012](../decisions/ADR-M012.md) (resource model — its §9 per-operation classification table is the **binding input** to the Apply column below), [ADR-W021](../decisions/ADR-W021.md) (REST verbs), [ADR-RT008](../decisions/ADR-RT008.md) (realtime), [ADR-P007](../decisions/ADR-P007.md) (PVW bus + monitors), [ADR-T015](../decisions/ADR-T015.md) (timing), [ADR-MV006](../decisions/ADR-MV006.md) (tally), [ADR-R011](../decisions/ADR-R011.md) (resilience). Paths use the canonical `/api/v1` base ([conventions §6](../architecture/conventions.md)); the bare `/v1` style elsewhere in this 2026-06-02 document predates it. Conventions that hold for **every** row: desired state lives in versioned config resources (`ETag`/`If-Match` → 412; 422 typed validation; `Idempotency-Key` accepted on every action); **live bus state is engine-owned** — mutating it goes exclusively through the bounded command bus drained at the engine's frame-boundary control seam, and is mirrored read-only (latest-wins registry + `switcher` topic events), so no row below can pace or back-pressure the engine (invariants #1/#10 by construction); every state-changing take has a `/plan` dry-run returning its class before apply (invariant #11); all durations are integer frame counts at the exact rational output cadence — `rate_ms` converts via exact rationals, never float fps (invariant #3). Hot = applied at the next frame boundary; **Hot (timed)** = starts at the next frame boundary, completes over its rate window (202 + operation id, outcome correlated on the event stream).

| Entity | Parameter | API | UI control | Apply | Notes |
|---|---|---|---|---|---|
| MixEffects (desired) | definition CRUD (program binding, flip-flop, default transition, USK definitions) | GET/POST/PATCH/DELETE /api/v1/switcher/mix-effects/{id} | Switcher > M/E setup | Hot (CRUD); structural add/remove = Hot via make-ready-then-swap | No engine contact until referenced; `validate_switcher()` cross-refs + M/E re-entry cycle check; structural add is admission-gated, demotes per ADR-M005 only if canvas format pins are touched. |
| MixEffects (live) | PVW crosspoint set | POST /api/v1/switcher/mix-effects/{id}/preview {source} | Panel PVW bus row; keys 1–9 | **Hot** | O(1) rebind precedent; cue/pre-warm is the arm — cold sources badge honest readiness ([ADR-P007](../decisions/ADR-P007.md)). |
| MixEffects (live) | PGM direct punch | POST /api/v1/switcher/mix-effects/{id}/program {source} | Panel PGM bus row; Shift+1–9 | **Hot** | Direct-punch = hot-cut semantics; tally recomputes same tick ([ADR-MV006](../decisions/ADR-MV006.md)). |
| MixEffects | cut | POST /api/v1/switcher/mix-effects/{id}/cut | CUT button + key | **Hot** | Immediate `200 {class, applied}`; PGM/PVW swap at the next frame boundary; cut during an in-flight transition completes it (jump to 1). AFV per source mode ([ADR-0059](../decisions/ADR-0059.md)). |
| MixEffects | auto | POST /api/v1/switcher/mix-effects/{id}/auto | AUTO button + key | **Hot (timed)** | 202 + operation id; resolves on `transition.completed`; progress = pure `f(tick.index)`; under overload AUTO demotes to CUT at arm/admission time — never shed mid-transition ([ADR-0055](../decisions/ADR-0055.md)). |
| MixEffects | T-bar position | POST /api/v1/switcher/mix-effects/{id}/tbar {position: 0..=10000} | T-bar drag | **Hot** | Conflated latest-wins **absolute** setter (one slot, never one command per sample); integer basis points on the wire, quantised half-up onto the engine's `u16` fixed-point position on apply; latch at full travel (10000) completes like auto ([ADR-W021](../decisions/ADR-W021.md)/[ADR-T015](../decisions/ADR-T015.md)). |
| MixEffects | transition type/rate/params | POST /api/v1/switcher/mix-effects/{id}/transition {kind, rate_frames\|rate_ms, params} | Transition selector + rate field | **Hot** | Arm-time configuration; wipe/DVE/stinger parameters per the [ADR-0055](../decisions/ADR-0055.md) taxonomy; stinger validated `trigger+mix ≤ duration` at arm. |
| MixEffects | next-transition arming {background, key[1..n]} | POST /api/v1/switcher/mix-effects/{id}/transition {elements} | Next-transition element buttons | **Hot** | Key-only transitions valid (background unarmed); armed USKs toggle on-air through the transition. |
| MixEffects | FTB | POST /api/v1/switcher/mix-effects/{id}/ftb {engaged: bool} | FTB button (engaged-state indicator) | **Hot (timed)** | Explicit-state, idempotent; a per-program master stage **after** the DSKs — the verb addresses it via the M/E that owns the program canvas (the M/E is the address, not the stage location; one M/E in MVP — [ADR-0056](../decisions/ADR-0056.md)/[ADR-W021](../decisions/ADR-W021.md)); own rate; optional audio-follow master ramp ([ADR-0059](../decisions/ADR-0059.md)); events `ftb.engaged`/`ftb.released`. Always available regardless of source health ([ADR-R011](../decisions/ADR-R011.md)). |
| MixEffects | plan (dry-run) | POST /api/v1/switcher/mix-effects/{id}/plan {action, …} | Pre-take class badge | RO/plan | Returns Class-1/Reset-lite/Class-2 without applying (the routing plan/take precedent; invariant #11). |
| USK (per M/E) | keyer parameters: type (luma\|linear\|chroma\|pattern\|dve_pip), fill/key sources, clip/gain/invert, garbage matte, key priority | PATCH /api/v1/switcher/mix-effects/{id} {keyers[…]} | Keyer panel (per-USK strip) | **Hot** | Trims (clip/gain/matte) are frame-boundary state; arming rides /transition; key math at pinned insertion points (luma on post-range-expand Y; chroma in linear RGB — [ADR-C007](../decisions/ADR-C007.md)); fill+key = second per-tile source reference ([ADR-0056](../decisions/ADR-0056.md)). |
| DSK (live) | on-air / off-air / auto / tie | POST /api/v1/switcher/downstream-keyers/{id}/on-air · /off-air · /auto · /tie {tied: bool} | DSK buttons | **Hot** (auto = Hot (timed)) | Explicit idempotent verbs (repeat = no-op 200); TIE joins the next background transition; AUTO mixes at the DSK's own rate ([ADR-0056](../decisions/ADR-0056.md)). |
| DSK (desired) | definition CRUD + clip/gain/matte | GET/POST/PATCH/DELETE /api/v1/switcher/downstream-keyers/{id} | DSK setup | Hot | Canvas-level, post-M/E, pre-FTB; composited **on-clock** in the engine render plan — never the off-thread monitoring overlay bake ([ADR-0054](../decisions/ADR-0054.md)). |
| TransitionPreset / WipePattern / Composition (desired) | CRUD | switcher collections per [ADR-W021](../decisions/ADR-W021.md) | Transition/wipe/composition editors | Hot | Typed, internally-tagged; takes effect at next arm; compositions are bus-selectable multi-box sources rendered as a same-tick pre-pass ([ADR-0056](../decisions/ADR-0056.md)). |
| AuxBus (live) | route | POST /api/v1/switcher/aux-buses/{id}/route {source\|tap} | Aux routing row | **Hot** (same rendition) / **Reset-lite** (IDR-aligned splice) / **Class-2** (format differs ⇒ make-before-break) | Two operations behind one verb ([ADR-M012](../decisions/ADR-M012.md) §9): selecting a *source* onto the aux bus's own composite = **Hot** (same rendition — the aux encoder keeps running, content re-points at a frame boundary); re-pointing the output to a different *tap/program* changes which encoded stream it consumes = **Reset-lite** (RT-12 IDR-aligned splice) or **Class-2** (format differs). Bus-tap selectors `{program, clean, preview, me<n>, aux<n>}` are reserved tokens ([ADR-M012](../decisions/ADR-M012.md) §6, [ADR-R010](../decisions/ADR-R010.md)). |
| AuxBus (desired) | CRUD | GET/POST/PATCH/DELETE /api/v1/switcher/aux-buses/{id} | Aux setup | Hot for existing outputs; admission-gated spin-up (202) | A new aux = a new independent encode (invariant #7); existing feeds untouched. |
| MediaAsset (library) | register/import/CRUD (stills, clips, audio) | GET/POST/PATCH/DELETE /api/v1/media/assets | Media library screen | Hot | Control-plane only; probe-at-import **reject-or-transcode** policy (alpha truth table; HEVC-alpha rejected/import-transcoded); alpha via NV12+A ([ADR-0057](../decisions/ADR-0057.md)/[ADR-0058](../decisions/ADR-0058.md)). |
| MediaPlayer (live) | load / cue | POST /api/v1/media/players/{id}/load · /cue | Player Load/Cue | **Hot, async readiness** | 202 until primed (decode-first-frame + pause, prime-gated); decode spawns off-thread; `media.player_state` events. |
| MediaPlayer (live) | play / pause / stop / seek | POST /api/v1/media/players/{id}/play · /pause · /stop · /seek {frame\|ms} | Transport buttons; timeline scrub | **Hot** | Gates the wall-clock pacer; `seek` is frame-accurate (ms via exact rationals, [ADR-T015](../decisions/ADR-T015.md)); the engine *samples* the player's last-good store per tick — a paused/underrunning player can never stall the output (inv #1/#2). |
| MediaPlayer (desired) | loop / EOF policy / play-on-take / in-out marks | PATCH /api/v1/media/players/{id} {loop, eof_policy, play_on_take, in, out} | Player inspector | Hot | EOF `hold_last_frame\|loop\|black\|auto_off`; play-on-take = roll-clip on the take that puts the player on PGM; marks are integers, ms via exact rationals ([ADR-0057](../decisions/ADR-0057.md)/[ADR-T015](../decisions/ADR-T015.md)). |
| Macro (live) | run | POST /api/v1/macros/{id}/run | Macros palette | **Hot** | 202 + operation id; control-plane sequencer replaying ordinary commands through the same bounded bus (never engine-side — inv #10); `corr` on `macro.started/step/completed`; failure = halt + event ([ADR-R011](../decisions/ADR-R011.md)). |
| Macro (desired) | definition CRUD (steps + wait_frames/wait_ms) | GET/POST/PATCH/DELETE /api/v1/macros/{id} | Macro editor | Hot | Internally-tagged step union; multi-op steps batch at one frame boundary (the open obs-websocket `SERIAL_FRAME` precedent). |
| Memory (snapshot) | recall (salvo + recall-scope masks) | POST /api/v1/salvos/{id}/arm · /take · /cancel (BUILT surface; recall body extended with `{scope: {sources, keyers, transition, audio}}` booleans, default all true) | Memories panel | **Hot** | Salvo-shaped atomic batch at one frame boundary; mixed recall (sources+keyers+transition+audio); scope-mask wire shape pinned in [ADR-M012](../decisions/ADR-M012.md); supersedes ADR-MV004's salvo-only automation story. |
| Source.audio | AFV mode | PATCH /api/v1/sources/{id} {audio.afv} | Source > Audio > Follow video | **Hot** | `fade_with_transition` (equal-power crossfade, integer sample budgets from the same transition window) \| `hard_cut` (zero-ramp at the switch point, badged not-pop-free); Source owns per-input audio attributes (§1.2, [ADR-0059](../decisions/ADR-0059.md)). |
| Switcher audio (program bus) | master gain / master mute envelope | PATCH /api/v1/switcher/audio {master_gain_db, master_mute} | Master fader + mute | **Hot** | One multiply pass in the program-bus mix; envelopes never folded into steady gain (gain-preserving); loudness-normaliser adaptation held during ramps; per-strip mute is a separate envelope — unmute restores exact prior gain (REST surface pinned in [ADR-W021](../decisions/ADR-W021.md); engine `MasterEnvelope` command per [ADR-0059](../decisions/ADR-0059.md)). |
| Switcher (live) | state mirror | GET /api/v1/switcher/state | Panel state; external control surfaces | RO | Engine-owned latest-wins registry; per-element booleans (source-on-pgm, keyer-on-air, ftb-active, media-playing) for gateway button feedback ([ADR-W021](../decisions/ADR-W021.md) §13). |
| Switcher realtime | events + conflated lanes | WS wss://[2001:db8::a]:8443/api/v1/ws — topic `switcher` (+ existing `tally`, `audio.meters`) | Panel live state | RO | Lossless lifecycle events in the replay ring; `transition.progress` publisher-side ~30 Hz conflated, excluded from replay, healed by connect-time snapshot frames ([ADR-RT008](../decisions/ADR-RT008.md)). |
| Bus monitors | PGM/PVW/clean stills | GET http://[::1]:8080/api/v1/preview/buses/{bus}.jpg | Monitor strip | RO | ~1 Hz JPEG stills first, honestly labelled; WHEP per-bus upgrade lane later; monitor lanes shed **before** any program rung ([ADR-P007](../decisions/ADR-P007.md)). |
| Warm-restart snapshot | enable + recall-on-start | control-plane setting (off by default) | Settings > Switcher > Warm restart | Hot (control-plane only) | Separate persisted resource, never part of config/export; replayed at startup as ordinary commands (one apply path); mid-transition restores as completed ([ADR-M012](../decisions/ADR-M012.md) §8). |
