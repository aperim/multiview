...
A reconnect must mint a new ticket (or accept a small reuse window). The subprotocol path must always echo mosaic.v1 (integration-tested) and is documented with its logging risk. Cookie auth requires an Origin allow-list. The in-proc ticket store does not span replicas — trait-gated for a future shared store.
