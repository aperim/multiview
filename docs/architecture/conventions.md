...
| `multiview-webrtc` | The shared **native WebRTC transport endpoint** (str0m): one dual-stack `[::]` UDP socket muxing **all** sessions (WHIP ingest, WHEP preview + output viewers, WHIP push), full-ICE with IPv6-first candidates **plus an in-crate sans-IO TURN client** (RFC 5389/5766/8656; str0m 0.16.2 ships none — ADR-0048 §5.1), per-run DTLS cert, one bounded driver task + session GC. The pure TURN/STUN codec + client are in the default build; only the socket wiring is `native`. The native transport relocated here from `multiview-preview` (ADR-0048). | `native` |
