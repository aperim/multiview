...
| **M1** — First pixels | Single source: decode → compose (software) → output | One input decoded, composited (software), served to file/RTSP, driven by the fixed-cadence **output clock** | 📋 Planned |
| **M2** — GPU path | GPU compositor + HW decode/encode + HAL negotiation | wgpu/Metal/CUDA compositor; NVDEC/NVENC, VideoToolbox, VAAPI/QSV; per-stage negotiation; NV12-throughout; zero-copy within a vendor island | 📋 Planned |
| **M3** — Bulletproof | Multi-source + output-clock + framestore + resilience | N inputs; last-good-frame stores + tile state machine; supervised reconnect; **never-falters** output-validity SLO holds under fault injection | 📋 Planned |
| **M4** — Full I/O + audio | HLS/LL-HLS/NDI/SRT + multistream audio | All input/output protocols; discrete per-input audio tracks + mixed program bus; correct A/V sync | 📋 Planned |
| **M5** — A/V features + color | Subtitles, overlays & color correctness | libass burn-in + passthrough; overlay layers (labels/clocks/meters/alert cards); the 4-axis color pipeline with output tagging + ffprobe verify | 📋 Planned |
| **M6** — Control API | Web API + OpenAPI | axum `/api/v1`, OpenAPI 3.1 + Scalar, RFC 9457 errors, auth/RBAC, SQLite, the engine command bus | 📋 Planned |
| **M7** — Management UX | Web app + preview + realtime | React app + drag-and-drop layout editor; WS/SSE + AsyncAPI; isolated preview (input/program/output) | 📋 Planned |
| **M8** — Efficiency | Efficiency, adaptive control & density | Decode-at-display-resolution; admission control + ordered degradation ladder; published density numbers per commodity tier | 📋 Planned |
