...
| **M13** — Production switcher | Live production-switcher layer **inside one program**: M/E (mix/effects) stages with PGM/PVW buses, cut/mix/dip/FTB transitions + T-bar, downstream keyers (fill+key, NV12+A alpha media), media library + players (cue/loop/play-on-take), audio-follow-video + master gain, internally-derived tally, macros + memories, switcher REST/WS + SPA operator panel | One M/E switches live under the output clock with frame-aligned takes and pop-free AFV audio; transition progress is a pure `f(tick)` (sampled, never pacing — inv #1/#10 hold under the chaos gates); 2 DSKs key alpha stills; players cue/loop/roll-on-take; derived red/green tally flows through the arbiter; plan/take API + keyboard-operable panel ship together | 📋 Planned |
...
## Production switcher extension (M13)

📋 **Planned — design pass complete (2026-06-11, docs-only).** One new milestone for the live **production-switcher layer**: M/E (mix/effects) stages, program/preview buses, transitions, keyers/FTB, media playout and switcher audio, all evaluated **per tick inside one program** so nothing in the switcher can pace or back-pressure the engine (invariants #1/#10 by construction — transitions are *sampled* at the output clock, durations are integer frame counts at the exact rational cadence, control rides the existing frame-boundary command drain). Vendor-neutral vocabulary and open references only (TSL UMD, AMWA NMOS IS-07, SCTE-35/104, obs-websocket, CasparCG AMCP, FFmpeg docs).

**Honest sequencing:** M13 builds on the M1–M8 core (engine clock + compositor drive, framestores, audio program bus, control plane/SPA) and **overlaps M10–M12 rather than following them** — its internally-derived tally feeds the same arbiter M11 wires for external TSL/IS-07 facts, and it depends on nothing in M12. The MVP boundary is one M/E (designed for N): PGM/PVW, cut/auto/T-bar, mix + dip + FTB with audio-follow, flip-flop, 2 downstream keyers (linear/luma, alpha stills via the NV12+A payload), media library + 2 players (stills + clips, loop, play-on-take, EOF policy), program audio AFV + master gain + meters, clean program tap, derived red/green tally through the arbiter, the switcher REST/WS surface with plan/take, the SPA panel with shortcuts, and macros + memories. Post-MVP: wipes, DVE, stinger transitions, chroma/pattern keyers, aux buses via the output crosspoint, preview-transition, multi-M/E, WHEP bus monitors, external control-surface/OSC/MIDI/TSL-egress adapters.

Design docs: the [production-switcher](docs/research/production-switcher.md) and [media-playout](docs/research/media-playout.md) briefs; decisions [ADR-0054…ADR-0059 + the eight companion area ADRs](docs/decisions/README.md#production-switcher) *(all Proposed)*; `SW-*` backlog in [docs/development/production-switcher-backlog.md](docs/development/production-switcher-backlog.md); detailed milestone plan in [docs/roadmap.md](docs/roadmap.md).
