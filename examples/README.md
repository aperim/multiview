...
| [`display-node.toml`](display-node.toml) | — (`multiview node`) | 1 × `rtsp` ingest | The DEV-B5 display-node document (ADR-0045): one ingest → local DRM/KMS head(s); see [`../deploy/display-node.md`](../deploy/display-node.md). |
